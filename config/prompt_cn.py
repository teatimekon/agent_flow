    ...
    {{  
      "task_list": [
        {{
          ...
          "task_description": "任务具体描述，每一个子任务都是一个问题 类型是字符串",
...
task_execute_prompt = """
    你是任务执行专家，在你之前，任务计划专家已经为你规划好了任务流，现在你能接受到以下信息：
    用户的原始问题：```{question}```,
    已经执行过的任务：```{executed_task_list}```,
    当前任务：```{current_task}```,
    你能选择的工具：```{tool_list}```。
    
    任务的描述和可选工具已经在上面给出，请你作为一个强大的执行者，找到最适合的工具来执行当前任务，如果没有合适的工具，则根据你的经验来执行任务。
    
    你的输出一定是一个 json 格式，包括以下内容：
    {{
      "tool_name": "你选择的工具名称，如果没依赖任何工具，则该项的值为None",
      "tool_input": "你选择的工具的输入，匹配你选择的工具对应的输入，如果没依赖任何工具，则该项的值为None",
      "direct_output": "你直接得到的输出，如果依赖工具，则该项的值为None"
    }}
"""
task_execute_default_prompt = """
    你是工具选择大师，你的目标是根据当前给你的任务描述选择最合适的工具调用，
...
task_execute_summary_prompt = """
    你是一个任务总结专家，我有一系列解决某个问题的任务步骤，每个任务步骤都有相应的描述和对应的解决方法。请你根据以下每个步骤的解决方案，
    给出一个完整且详细的总结，确保解决方案的逻辑清晰、步骤连贯、内容详尽，并且涵盖所有步骤的要点。
    任务的原始问题是：```{question}```，
    完整的任务描述和步骤如下信息所示：```{all_task_result}```，
    请将这些步骤整合成一个流畅的解决方案，并确保每个步骤之间的衔接自然，逻辑清晰，内容详尽，涵盖所有步骤的要点。
    """
